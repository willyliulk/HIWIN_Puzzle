...
// HIWIN_Puzzle.cpp: 定義應用程式的類別表現方式。
//
...
BEGIN_MESSAGE_MAP(CHIWINPuzzleApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CHIWINPuzzleApp::OnAppAbout)
	// 依據文件命令的標準檔案
...
CHIWINPuzzleApp::CHIWINPuzzleApp() noexcept
{
	// 支援重新啟動管理員
	...
	SetAppID(_T("HIWINPuzzle.AppID.NoVersion"));

	// TODO: 在此加入建構程式碼，
	// 將所有重要的初始設定加入 InitInstance 中
}

// 唯一一個 CHIWINPuzzleApp 物件

CHIWINPuzzleApp theApp;


// CHIWINPuzzleApp 初始化

BOOL CHIWINPuzzleApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CHIWINPuzzleDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架視窗
		RUNTIME_CLASS(CHIWINPuzzleView));
	if (!pDocTemplate)
		return FALSE;
...
int CHIWINPuzzleApp::ExitInstance()
{
	// TODO: 處理其他您已經加入的資源
	AfxOleTerm(FALSE);
...
// 執行對話方塊的應用程式命令
void CHIWINPuzzleApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
